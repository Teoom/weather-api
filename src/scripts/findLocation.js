  ...
  return async (position) => {
    ...
    try {
      ...
      if (resposne[0].ok && resposne[1].ok) {
        ...
        elements.locationInfo.date.textContent = `${namesOfDay[indexOfDay]}, ${day} ${namesOfMonths[month-1]}`;

        const { temp_c, feelslike_c, humidity, wind_kph, pressure_mb, uv} = weather.current;
        const { sunrise, sunset} = weather.forecast.forecastday[0].astro;

        String(temp_c).includes("-") ? elements.currentWeather.temp.style.fontSize = "70px" : elements.currentWeather.temp.style.fontSize = "80px";
       ...
       elements.currentWeather.figcaption.textContent = weather.current.condition.text;
      ...
      const dailyForecasy =  weather.forecast.forecastday.filter((item, index) => index !== 0);
